...
- **참고 URL**: 계약서, 구글드라이브 문서, 노션 페이지 등 외부 링크를 텍스트로 입력하면
  기록 카드에 "참고 URL 열기" 링크로 표시됩니다. (파일 업로드가 아니라 링크 저장이라 용량 제한 없음)
- **작성자 기록**: 등록/수정 시 "작성자" 이름을 입력하면 기록 카드에 "등록: 이름"이 표시되고,
  다른 사람이 수정하면 "최근 수정: 이름 (날짜)"가 추가로 표시됩니다.
  입력한 이름은 브라우저에 저장되어 다음에 다시 입력할 필요가 없습니다.
